...
class TestTransform(unittest.TestCase):
    ...
    def test_label_encoder(self):
        ...
        for split in y_encoded_splits:
            for array in split:
                self.assertIsInstance(array, numpy.int64)
    ...
    def test_onehot_and_ordinal_encoder(self):
        ...
        for split in x_encoded_splits:
            for col in split:
                for array in col:
                    self.assertIsInstance(array, numpy.float64)
    ...
    def test_df_transform(self):
        ...
        self.assertEqual(len(df_transformations), 3)
        for trans_df in df_transformations:
            self.assertIsInstance(trans_df, pd.core.frame.DataFrame)
